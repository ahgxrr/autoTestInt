...
public class AddUserTest {
   ...
   // @Test(description = "添加用户接口接口")
    public void addUser() throws IOException, InterruptedException {

        SqlSession session = DatabaseUtil.getSqlSession();
        AddUserCase addUserCase = session.selectOne("addUserCase",1);
        System.out.println("addUserCase: "+addUserCase.toString());
        //从数据表addUserCase中拿到了一条用户数据：zhao9
        //一会而把这个用户输入接口，看得到的响应与这条用户数据的expected是否一样
        System.out.println("addUserUrl: "+TestConfig.addUserUrl);

        //下边的代码为写完接口的测试代码
        String result = getResult(addUserCase);//从接口中拿到的，返回的是true，
        // 只要和写的接口里的请求用户一模一样,result就是true
